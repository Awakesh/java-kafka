...
@Component
public class DmpNbaApiTransformer {

    private static final Logger uncaughtExceptionLogger = LoggerFactory.getLogger("CustomUncaughtExceptionLogger");

    @Value("${kafka.stream.dmp_nba_di_api.id}")
    private String streamAppId;

    @Value("${kafka.stream.dmp_nba_di_api.thread}")
    private String threadCount;

    @Autowired
    DmpNbaApiTransformerTopology dmpNbaApiTransformerTopology;
    ...
    public void init() throws IOException {
        ...
        Topology topology = dmpNbaApiTransformerTopology.builder(avroConfig, false);
